...
#PPLS
cred = credentials.Certificate('/home/ronny/gps-geodesi/asset/ppls-shift-firebase-adminsdk-61y26-a26269269d.json')
        ...
        if callout == 1:
            ...
            if run_status == 1:
                ...
                with serial.Serial(device_port, 115200, timeout=2) as ser:
                     send_ubx_message(ser, b'\xB5\x62\x06\x02\x03\x00\x02\x15\x01\x21\x91')
                     send_ubx_message(ser, b'\xB5\x62\x06\x01\x03\x00\x02\x13\x01\x1F\x97')
                now = datetime.now()
                ...
                convert_to_rinex()
                convert_to_rinex2()
                shutil.copy("/home/ronny/gps-geodesi/output.obs","/home/ronny/gps-geodesi/output1.obs")
                shutil.copy("/home/ronny/gps-geodesi/output.nav","/home/ronny/gps-geodesi/output1.nav") 
                ...
                data = {
                    f"{tanggal}": dropbox_destination_path
                }
                ref.update(data)

                ref = db.reference(f"/Realtime/base/Storage/nav")
                data = {
                    f"{tanggal}": dropbox_destination_path_nav
                }
                ref.update(data)
                run_status = 0
                ref = db.reference(f"/Realtime/base/")
